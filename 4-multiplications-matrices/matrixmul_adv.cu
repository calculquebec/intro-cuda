...
void MatMul(float* A, float* B, float* C) {
    ...
    **TODO: Allocate space for A in device memory
    // Copy matrix A to device
    **TODO: Copy A matrix from host to device

    float *d_B = 0;
    // Allocate space for matrix B on device
    **TODO: Allocate space for B in device memory
    // Copy matrix B to device
    **TODO: Copy B matrix from host to device
    ...
    **TODO: Copy C matrix from device to host
    ...
    // Free device memory
    **TODO: Free all allocated device memory
}
...
__global__ void MatMulKernel(float* A, float* B, float* C) {
    ...
    **TODO: Compute the thread indexes: int col = ??? row = ???
